...
if(isset($_POST['token']) && !empty($_POST['token']) && isset($_POST['id']) && !empty($_POST['id'])){
    ...
    header('Location: /@tafel/changepass.php?error=1');
}
if(isset($_POST['p']) && !empty($_POST['p'])){
    $password = preg_replace("/[^a-zA-Z0-9]+/", "", $_POST['p']);
}else{
    header('Location: /@tafel/changepass.php?error=2');
}
...
if($stmt->rowCount() == 1){
    header('Location: /@tafel/changepass.php?status=1');
}else{
    header('Location: /@tafel/changepass.php?status=2');
}
